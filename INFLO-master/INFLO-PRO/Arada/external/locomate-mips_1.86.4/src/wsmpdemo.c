...
enum {BOOL, UINT8, UINT16, UINT32, UINT64, STRING, STRINGLONG, ADDR_MAC, ADDR_IPV4, ADDR_IPV6, RATESET};
...
enum {UNSECURED, SECURED, ENCRYPTED};//SECURITY TYPE
...
enum {MAIN, APPREG, WBSS, WSMP, WRSS, GET, SET, PROVIDER, USER, PSID ,LOG};
enum {RXPKT, TXPKT,NOTIFIND, WRSSREP, CBJ, ARGS, TSF};
...
static uint16_t listenlogport  =  9876; /*NOTE*/ 
...
void usage() {
	...
	fprintf(stderr,"                Launch wsmpdemo for operations on a target machine with ip address 192.168.1.96\n");
	...
	fprintf(stderr,"                where 192.168.1.82 is the IP address of one of the interfaces on local machine.\n\n");
	exit(1);
}
...
/*TODO*/
	...
	cmp = (rxpkt.chaninfo.channel == wsmrxind.chaninfo.channel) && (rxpkt.chaninfo.txpower == wsmrxind.chaninfo.txpower)
		&& (rxpkt.psid == wsmrxind.psid) && (rxpkt.version == wsmrxind.version) && (rxpkt.security == wsmrxind.security)
...
outrx:				
	...
	clr_status_msg();
	if(overIP == 0)	sprintf(status_msg, "%s(WSM) Received (%u Bytes)", (gpsrcvd)?" GPS": " ", 9 + rxpkt.data.length);
...
//	WMEApplicationRequest appRegRequest;
	...
	switch(action) {
		...
		case WME_DEL_PROVIDER:
			...
			sprintf(status_msg, "Provider %sRegistration %s", (add == 1)?" " : "Un",(result < 0)?"Failed" : "Successful" );
		...
		case WME_DEL_USER:
			...
			sprintf(status_msg, "User %sRegistration %s", (add == 1)?" " : "Un",(result < 0)?"Failed" : "Successful" );
...
void fill_board()
{
	...
	if(overIP == 0) {
		...
		if(!dofwd) {
			...
			strncpy(board[22] + 2, msg, msglen);		
		        if(ipvfour == 1)
				sprintf(msg,"%s:%u", inet_ntop(AF_INET, &their_addr_fwd4.sin_addr, str, sizeof(str) ), ntohs(their_addr_fwd4.sin_port));
			else if(ipvfour == 0)
				sprintf(msg,"%s:%u", inet_ntop(AF_INET6, &their_addr_fwd6.sin6_addr, str, sizeof(str) ), ntohs(their_addr_fwd6.sin6_port));
			msglen = strlen(msg);	
			strncpy(board[23] + 2, msg, msglen);
		}
	}
...
//	static int packetnum = 0;
		...
		*(board[15] + 27 + i) = wsmrxind.data.contents[i];
		...
		if(gpsrcvd) {
			...
			sprintf(msg,"Year:%uMon:%udate:%u ",year_val,month_val,day_val );
                        ...
                        sprintf(msg,"Long :%d %c", longitude_val, (longitude_val >0)? 'E' : 'W' );                        msglen = strlen(msg);
                        strncpy(board[12] + 27, msg, msglen);
	...
	if (overIP > 0) {
		...
		addwsmp.macaddr[0],addwsmp.macaddr[1],addwsmp.macaddr[2],addwsmp.macaddr[3],addwsmp.macaddr[4],addwsmp.macaddr[5]);
	} else {
