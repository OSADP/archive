...
int AsnLog(int from_txrx, uint8_t pktnum, int msgType, int logFormat, char *buf, void *asnData, void *wsm, double time ,uint16_t sec_16)
{
    static int seq = 0;
    static int inuse = 0;
...
//.....logfmt pcap ...
	...
	if( logFormat == CSV ) 
	 ...
	 sprintf(m, "<channel=%u> <rateindex=%u> <txpower=%u> ", wsmind->chaninfo.channel, wsmind->chaninfo.rate, wsmind->chaninfo.txpower);
         sprintf(buf + i, "%s ", m);
         i += (strlen(m));
	...
	if(msgType == WSMMSG_BSM) {
	  ...
	  if(bsm_data->status != NULL)
	  {
	     struct FullPositionVector *fpv = (struct FullPositionVector *)(bsm_data->status->fullPos);
	         ...
	         if(fpv->utcTime !=NULL){
	             sprintf(m, "<UTC TIME: Year=%ld Month=%ld Day=%ld Hour=%ld Min=%ld sec=%ld> ", *fpv->utcTime->year,*fpv->utcTime->month,*fpv->utcTime->day,*fpv->utcTime->hour,*fpv->utcTime->minute,*fpv->utcTime->second);
                     sprintf(buf + i, "%s ", m);
                     i += (strlen(m));
	         }
	         ...
	         if(fpv->elevation !=NULL){
                     sprintf(m, "<elevation=%2u> ", (BIGENDIAN)?(*(uint16_t *)fpv->elevation->buf):swap16(*(uint16_t *)fpv->elevation->buf));
                     sprintf(buf + i, "%s ", m);
                     i += (strlen(m));
 	         }
             
	         if(fpv->speed!=NULL){
	             sprintf(m, "<speed=%hd> ", (BIGENDIAN)?(*(uint16_t *)fpv->speed->buf):swap16(*(uint16_t *)fpv->speed->buf));
                     sprintf(buf + i, "%s ", m);
    	             i += (strlen(m));
                 }
	         if(fpv->heading !=NULL){
     ...
     if( logFormat == XML ) 
     {
	  ...
	  if(bsm_data->status != NULL)
	  {
         ...
         if(fpv != NULL){
	         ...
	         if(fpv->Long !=NULL) {
                     sprintf(m, "<longitude> %ld </longitude> <londir> %c </londir> \n", fpv->Long, (fpv->Long < 0)? 'W' : 'E');
                     sprintf(buf + i, "%s ", m);
                     i += (strlen(m));
	         }
	     
	         if(fpv->lat !=NULL) {
                     sprintf(m, "<latitude> %ld </latitude> <latdir> %c </latdir> \n", fpv->lat, (fpv->lat < 0)? 'S' : 'N');
                     sprintf(buf + i, "%s ", m);
                     i += (strlen(m));
                 }

	         if(fpv->elevation !=NULL) {
                     sprintf(m, "<elevation>%2u <elevation> ", (BIGENDIAN)?(*(uint16_t *)fpv->elevation->buf):swap16(*(uint16_t *)fpv->elevation->buf));
                     sprintf(buf + i, "%s ", m);
                     i += (strlen(m));
	         }
