...
int ChannelAcess = CHACCESS_CONTINUOUS; //1;
...
extern int RSU_Flag = 0;
extern int GPS_Fix_Flag = 0;
extern int RVbsm;
extern int rssi;
int RSUrssi;
time_t RSU_Timer;
time_t Curr_Timer;
time_t TIM_Timer; 
time_t GPS_Timer; //Timer to toggle GPS Status bit
time_t IdRandomization_Timer;
extern wtr btwtr;
extern veh btveh;

// Bluetooth
sdp_session_t *session1 = 0;
...
int register_app() {
	...
	if (waveappmode == USER) {
		...
		if (ChannelAcess > 1) {
			...
			channelaccess = CHACCESS_CONTINUOUS;// _ALTERNATIVE;
		} else {
...
int main(int argc, char *argv[]) {
	...
	/*
	FILE* pGpsSpoofFile = fopen("/var/bin/spoof_gps_data.txt", "r");
	if(pGpsSpoofFile)
	{
	    printf("Using spoof GPS data...\n");
	    ...
	    int count = fscanf(
	        ...
	        &spoofGpsData.latitude,
	        &spoofGpsData.longitude,
	        &spoofGpsData.course,
	        &spoofGpsData.speed,
	        &spoofGpsData.altitude
        );
        ...
        if(count != 5)
        {
            printf("Error reading spoof gps data\n");
        }
        
        fclose(pGpsSpoofFile);
	}
	else
	{
	    printf("Could not find spoof_gps_data.txt");
	}
	*/
	...
	gps_enableSpoofDevice("/var/bin/spoofs/gps-spoof.txt");
	...
	if (MessageType[0] == DSRCmsgID_basicSafetyMessage || MessageType[1] == DSRCmsgID_travelerInformation) {
		printf("Message Type BSM Or TIM\n");
		//printf("\ncertChangeFlag = 1\n");
		certChangeFlag = 1;
	}
...
//	}
	...
	ret = pthread_create(&btServer, NULL, bt_server, NULL );
	if (ret) { }
	sched_yield();
...
void *rx_client(void *data) {
	...
	time(&RSU_Timer);
	time(&TIM_Timer);
	time(&IdRandomization_Timer);
	RVbsm = 0;
	RSUrssi = 0;
	rssi = 0;
	initializeVEH(&btveh);
	initializeWtr(&btwtr);
	ConfigFile gpConfigFile;
	...
	while (1) {
		...
		if (ret > 0) {
			...
			cfEdit(&gpConfigFile);
			if (!gpConfigFile.appVehicleIdLock &&  difftime(Curr_Timer, IdRandomization_Timer) >= 300.0)
			{
				printf("Changing appVehicleID\n");
				time(&IdRandomization_Timer);
				gpConfigFile.appVehicleId = rand() << 16 | rand();
			}
			cfUpdate(&gpConfigFile);
			//mysleep( 0, 5000000);
		}
		sched_yield();
		//mysleep(0, 10000000);
...
int RxpktProcess() {
	//Check if it is from RSU.
	uint8_t macaddr[IEEE80211_ADDR_LEN];
	int i, counter, k = 0;
	int rsuMessage = 0;
	
	//printf("MAC: %d : %d : %d : %d : %d : %d\n\n", rxpkt.macaddr[0], rxpkt.macaddr[1], rxpkt.macaddr[2], rxpkt.macaddr[3], rxpkt.macaddr[4], rxpkt.macaddr[5]);
	while (k < (cfGetConfigFile()->rsuNum))
	{
		extract_macaddr(macaddr, cfGetConfigFile()->rsuMacAddr[k]);
		for(counter = 0; counter < IEEE80211_ADDR_LEN; counter++) {
			//printf("%X",rxpkt.macaddr[counter]);
			if(macaddr[counter] != rxpkt.macaddr[counter]) {
	   			RSU_Flag = 0;
				break;
			}
		}
		if (counter == IEEE80211_ADDR_LEN) {
			printf("\n\nRSU rssi: %d\n\n", rxpkt.rssi);
			RSUrssi = rxpkt.rssi;
			rssi = RSUrssi;
//		uiprintf("RSU rssi: %d\n", rxpkt.rssi);
			if(RSUrssi >= cfGetConfigFile()->rsuCellNetTransRSSI ) {
				RSU_Flag = 1;
				time(&RSU_Timer);
			}
		}
	...
	if (thread_options & RXALL_MASK) {
		int Content_type = AsmDecodeContentType(&rxpkt);
		...
		if (!rxmsg.decode_status) {
			...
			if(rxmsg.type == WSMMSG_BSM) {
				...
				RVbsm++;
				rssi = rxpkt.rssi;
				//Queued_State = bsmLog->blob1.buf[34] & 0x20;
				//printf("queued_state: %d\n",Queued_State);
			} 
			else if (rxmsg.type == WSMMSG_TIM || DSRCmsgID_travelerInformation && RSU_Flag)
			{
				VehicleState pState_out;
				qwarnGetCurrentVehicleState(&pState_out);
				Vector currHeading;
                		mapHeadingToVector(pState_out.heading_rad, &currHeading);

				int j,l, inSameDirection = 0;
				TravelerInformation_t * tim = (TravelerInformation_t *)rxmsg.structure;
				for (j = 0; j < tim->dataFrames.list.count; j++)
				{ 
					for (l = 0; l < tim->dataFrames.list.array[j]->regions.list.count; l++)
					{
						ValidRegion_t* pValidRegion = tim->dataFrames.list.array[j]->regions.list.array[l];
						...
						if (vecDot(&alertHeading, &currHeading) > 0.1)
						{
							inSameDirection = 1;
							break;
						}
			...
			qwarnOnRecvMessage(rxmsg.structure, rxmsg.type);
...
//  sem_wait(&can_sem);
	...
	if (ALL == 1) {
		...
		sts->lights[0] = btveh.headlight_status; //candata.exteriorLights;
		/*int cnt, mv = 0;
		uint16_t bit = 0x1;
...
*/
	 ...
	 sts->wipers->statusFront.buf[0] = btveh.wiper_status;
/*
...
//	btUpdateGpsData(&wsmgps);
	...
	brake_status = brake_status | (btveh.brake_status << 0); //Brakes Status
	brake_status = brake_status | (btveh.brake_applied << 4);	//Brakes Applied
	brake_status = brake_status | (qwarnIsQueued() << 5); 			//Queued State
	brake_status = brake_status | (btveh.traction_control_status << 6); 	//Traction Control State
	brake_status = brake_status | (btveh.anti_lock_braking_status << 8);	//Anti Lock Braking Status
	brake_status = brake_status | (btveh.stability_control_status << 10);	//Stability Control Status
	brake_status = brake_status | (btveh.brake_boost_status << 12);		//Brake Boost Status
	brake_status = brake_status | (btveh.auxiliary_brake_status << 14);	//Auxiliary Brake Status
/*
	int cnt, mv = 0;
	uint16_t bit = 0x1;
...
*/
	//Storing humidity in RainData
	 ...
	 rst->buf[0] = (uint8_t) (btwtr.hum + .5); //round to the nearest integer //RainSensor_none;
	 sts->rainData = rst;
 
	 sts->airTemp = (AmbientAirTemperature_t *) calloc(1,sizeof(AmbientAirTemperature_t));
	if(btveh.external_air_temperature > -1000) {
	 	sts->airTemp[0] = btveh.external_air_temperature;
		printf("airTemp: %ld\n",sts->airTemp[0]); 
	} else {
		sts->airTemp[0] = (long) (btwtr.temp + .5);
	}

	 sts->airPres = (AmbientAirPressure_t *) calloc(1,sizeof(AmbientAirPressure_t));
	if(btveh.barometric_pressure > 0) {
		 sts->airPres[0] = btveh.barometric_pressure; 
	} else {
		sts->airPres[0] = (long) (btwtr.pres + .5);
	}
	 ...
	 sts->steering->angle.buf[0] = btveh.steering_wheel_angle;//55;
/*
...
*/
	 ...
	 sts->steering->rate[0] = btveh.rate_of_change_of_steering_wheel; //115;
/*
	...
	if (ALL == 1) {
		sts->throttlePos = ( ThrottlePosition_t *) calloc(1,sizeof(ThrottlePosition_t));
		 sts->throttlePos[0]= btveh.throttle;
/*
		 speedConfidence = (SpeedandHeadingandThrottleConfidence_t *)calloc(1,sizeof(SpeedandHeadingandThrottleConfidence_t));
		 speedConfidence->buf = (uint8_t *) calloc(1,sizeof(uint8_t));
		 speedConfidence->size = sizeof(uint8_t);
...
//#if(ENABLED)
	...
	if (wsmgps.longitude == GPS_INVALID_DATA)
		...
		j = (long) ((wsmgps.longitude) * 10000000);
	intg32 = htobe32(j);
	...
	stw = btveh.steering_wheel_angle; //default value for Steering Wheel Angle 
	memcpy(bsm->blob1.buf + 25, &stw, 1);
	if (wsmgps.speed == GPS_INVALID_DATA)
		lon_acc_accl = 2001; //default value for longitude accleration
	if(btveh.longitudinal_acceleration != -1) {
		sintg16 = btveh.longitudinal_acceleration;
	} else {
		sintg16 = 2001/100; //sintg16 = lon_acc_accl;
	}
	sintg16 = htobe16(sintg16);
	memcpy(bsm->blob1.buf + 26, &sintg16, 2); //longi acc	
	if(btveh.lateral_acceleration != -1) {
		sintg16 = btveh.lateral_acceleration;
	} else {
		sintg16 = 2001/100;
	}
	sintg16 = htobe16(sintg16);
	//tmpintg16 = btveh.lateral_acceleration;//tmpintg16 = 2001; //default value for latitude accleration
	...
	if(btveh.yaw_rate != -32768) {
		yaw_rate = btveh.yaw_rate;
	} else {
	...
	tmpintg16 = 0x0000;
	tmpintg16 = tmpintg16 | btveh.brake_status; //Brakes Status
	tmpintg16 = tmpintg16 | (btveh.brake_applied << 4);	//Brakes Applied
	tmpintg16 = tmpintg16 | (qwarnIsQueued() << 5); 			//Queued State
	tmpintg16 = tmpintg16 | (btveh.traction_control_status << 6); 	//Traction Control State
	tmpintg16 = tmpintg16 | (btveh.anti_lock_braking_status << 8);	//Anti Lock Braking Status
	tmpintg16 = tmpintg16 | (btveh.stability_control_status << 10);	//Stability Control Status
	tmpintg16 = tmpintg16 | (btveh.brake_boost_status << 12);		//Brake Boost Status
	tmpintg16 = tmpintg16 | (btveh.auxiliary_brake_status << 14);	//Auxiliary Brake Status
/*	int cnt, mv = 0;
...
//printf(">a%lf gt%lf p%lf m%d t%llu s%u r%llu d%llu\n",wsmgps.actual_time,wsmgps.time, prev_acttime, msecs,tsfTime,sec_16, tsfref, tsfdiff);
	...
	if(btveh.speed != 8191) {
		transmission_speed[0] = (int8_t) (((uint32_t) (btveh.speed * 50) & 0xFF00) >> 8);
		transmission_speed[1] = (uint8_t) (((uint32_t) (btveh.speed * 50) & 0x00FF));
...
void sig_int(void) {
	...
	pthread_cancel(btServer);
	btDestroy();
	printf("Bluetooth shut down\n");
...
void ReadGpsdata() {
    ...
    {
    	gpsDevice_getData(gps_getDefaultDevice(), &wsmgps);

        //wsmgps = cfGetConfigFile()->spoofGpsData;
		
		GPS_Fix_Flag = wsmgps.fix;
        btUpdateGpsData(&wsmgps);
        qwarnUpdateGpsData(&wsmgps);
    ...
    else
    {
	    ...
	    if (gpssockfd > 0) {
			...
			if ((wsmgps.course != GPS_INVALID_DATA) && (wsmgps.speed != GPS_INVALID_DATA) && (wsmgps.latitude != GPS_INVALID_DATA) && (wsmgps.longitude != GPS_INVALID_DATA)) {
				if(wsmgps.speed < cfGetConfigFile()->headingLockSpeed)
				{
