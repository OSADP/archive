...
int parseTXConfigData(WSMRequest *wsmtxreq) 
{
	...
	int temp;
	float tempf;
	enum {POWER = 1, RATE, CHAN, DELAY, VAP};
...
int build_gps_wsmpacket(int sockfd,WSMRequest *wsmtxreq, GPSData *gpsdata, GPS_PACKET gpspkt)
{
	...
	if ( gpsdata->time > 0) {
		sprintf(wsmtxreq->data.contents + pos,"STR TIM %lf LAT %lf LON %lf ALT %lf", gpsdata->time, gpsdata->latitude, gpsdata->longitude, gpsdata->altitude );
