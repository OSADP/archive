...
msg_create_sign_msg(
	...
	UINT8 *app_data,
        int* buffSize,
	int app_length,
	UINT8 CertificateType, unsigned int *psid,int psidLen,UINT8 generationLocation, long generation_latitude,long generation_longitude)
{
    // data to be signed
    ...
    *buffSize = sizeof(AsmMsg_Sign_Msg_Req1_t)
    ...
    req2 = (AsmMsg_Sign_Msg_Req2_t*)&buff[*buffSize - psidLen/*sizeof(AsmMsg_Sign_Msg_Req2_t)*/ -  sizeof(AsmMsg_Sign_Msg_Req3_t)];
...
msg_decode_certificate(UINT8* signdata, UINT8* Algo,UINT16* certlength)
{
    ...
    certificate_sublength = sizeof(cert->scope_ntlocalized.sname) + cert->scope_ntlocalized.sname.length_subject_name - sizeof(cert->scope_ntlocalized.sname.subj_name[0]);
    
    certificate_perlength = *(intpo + certificate_flength + certificate_sublength + sizeof(cert->scope_ntlocalized.type));
    
    certificate_klength =  *(intpo + certificate_flength + certificate_sublength + certificate_perlength + sizeof(cert->scope_ntlocalized.type) + sizeof(cert->scope_ntlocalized.length_permission_field) + sizeof(cert->Expiration)+sizeof(cert->crl_series));

    *certlength = sizeof(Asm_certificate) - sizeof(cert->scope_ntlocalized.sname) + certificate_sublength - sizeof(cert->key) + certificate_klength;
    *Algo = *(intpo + certificate_flength + certificate_sublength + certificate_perlength + sizeof(cert->scope_ntlocalized.type) + sizeof(cert->scope_ntlocalized.length_permission_field) +sizeof(cert->Expiration) + sizeof(cert->crl_series) + sizeof(cert->length_public_key));
}
...
msg_decode_sign_msg(UINT8* app_data,UINT8* signdata,int *recv_size,UINT8 *Algo,int send_size)
{
	...
	res2 = (AsmMsg_Extraction_Signed_Msg2*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type.ctchain.ct[0])];
    }
	...
	certlength = 8;
	res2 = (AsmMsg_Extraction_Signed_Msg2*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type)];
    }
	...
	if(certtype == SIGNER_TYPE_CERT_CHAIN)
	    res3 = (AsmMsg_Extraction_Signed_Msg3*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type.ctchain.ct[0])+sizeof(AsmMsg_Extraction_Signed_Msg2)];
	else
	    res3 = (AsmMsg_Extraction_Signed_Msg3*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type)+sizeof(AsmMsg_Extraction_Signed_Msg2)];
	buflength += sizeof(AsmMsg_Extraction_Signed_Msg2);
    } else {
	mfpresense = 0;
	if(certtype == SIGNER_TYPE_CERT_CHAIN)
	    res3 = (AsmMsg_Extraction_Signed_Msg3*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type.ctchain.ct[0])+sizeof(AsmMsg_Extraction_Signed_Msg2) - 1];
	else
	    res3 = (AsmMsg_Extraction_Signed_Msg3*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type)+sizeof(AsmMsg_Extraction_Signed_Msg2) - 1];
	buflength += (sizeof(AsmMsg_Extraction_Signed_Msg2)-1);
    }

    // Caluculating the application data length and extracting the app data
    *recv_size = ((res3->app_length[0] << 24) | (res3->app_length[1] << 16) | (res3->app_length[2] << 8) | (res3->app_length[3]));
    ...
    if(certtype == SIGNER_TYPE_CERT_CHAIN) {
	if(mfpresense)
	    res4 = (AsmMsg_Extraction_Signed_Msg4*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type.ctchain.ct[0])+sizeof(AsmMsg_Extraction_Signed_Msg2)+sizeof(AsmMsg_Extraction_Signed_Msg3)-sizeof(res3->app_data[0])+*recv_size];
	else 
	    res4 = (AsmMsg_Extraction_Signed_Msg4*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type.ctchain.ct[0])+sizeof(AsmMsg_Extraction_Signed_Msg2)-1+sizeof(AsmMsg_Extraction_Signed_Msg3)-sizeof(res3->app_data[0])+*recv_size];
    } else { 
	if(mfpresense)
	    res4 = (AsmMsg_Extraction_Signed_Msg4*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type)+sizeof(AsmMsg_Extraction_Signed_Msg2)+sizeof(AsmMsg_Extraction_Signed_Msg3)-sizeof(res3->app_data[0])+*recv_size];
	else
	    res4 = (AsmMsg_Extraction_Signed_Msg4*)&signdata[sizeof(AsmMsg_Extraction_Signed_Msg1)+certlength-sizeof(res1->cert_type)+sizeof(AsmMsg_Extraction_Signed_Msg2)-1+sizeof(AsmMsg_Extraction_Signed_Msg3)-sizeof(res3->app_data[0])+*recv_size];
    }
...
msg_create_verify_msg(
        ...
        UINT8* buff,
        int* buffSize,UINT32 msgValidityDistance,UINT8 detectReplay,UINT8 generationLocationValidity,long local_latitude,long local_longitude)
{
    // request structures
		    ...
		    UINT16 wsalength,UINT32 *wsa_config,UINT32 latitude,UINT32 longitude,UINT16 elevation)
{
    ...
    *buffSize = sizeof(AsmMsg_Sign_WSA_Req1_t)
    ...
    req2 = (AsmMsg_Sign_WSA_Req2_t*)&buff[*buffSize - sizeof(AsmMsg_Sign_WSA_Req2_t) - psidLength  +  sizeof(req2->permissions)];
        ...
        UINT16* buffSize,UINT32 *wsa_config,UINT32 latitude,UINT32 longitude)
{
    ...
    *buffSize = sizeof(AsmMsg_Verify_WSA_Req1_t)
   	...
   	req2 = (AsmMsg_Verify_WSA_Req2_t*)&buff[*buffSize - sizeof(AsmMsg_Verify_WSA_Req2_t) -  psidLength +  sizeof(req2->permissions)];
   	req2->perform_cryptographic_verification = TRUE;
