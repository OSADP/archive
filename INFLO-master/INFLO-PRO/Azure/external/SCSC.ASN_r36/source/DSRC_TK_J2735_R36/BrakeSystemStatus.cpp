...
void BrakeSystemStatus_Init ( BrakeSystemStatus_t* theObj)
{
	if (theObj != NULL)
	{
		// Allocate memory needed on heap
		int bufSize = BrakeSystemStatus_BuffSize;  
		theObj->buf = (uint8_t *) malloc(bufSize);
		assert(theObj->buf);
		theObj->size = bufSize;
		// Set to unavailable as per rules in SAE DSRC
		// see clause 6.8 Data Element: DF_BrakeSystemStatus
		// first 5 bit all zero (no brakes active and spare=zero), 
		// rest in sets of two are set as zero for 'unavailable'
		// thus, all default values are in fact set to zero
...
void BrakeSystemStatus_ToBlob ( BrakeSystemStatus_t* theObj, char* theBlob)
{
...
void BrakeSystemStatus_FromBlob ( BrakeSystemStatus_t* theObj, char* theBlob)
{
	if ((theObj != NULL) && (theBlob != NULL))
	{
	}
