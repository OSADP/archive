...
INTEGER_st_prealloc(INTEGER_t *st, int min_size) {
	...
	if(p) {
		...
		st->size = 0;
		//st->buf = p;
		// DCK mod, above line replaced with below to add cast and make legal
		st->buf = (uint8_t *)p; // DCK Cast
		FREEMEM(b);
		return 0;
	} else {
		return -1;
...
INTEGER_decode_uper(asn_codec_ctx_t *opt_codec_ctx, asn_TYPE_descriptor_t *td,
	...
	// DCK mod, above line replaced with below to add cast
	if(ct && ct->flags & APC_EXTENSIBLE) {
	...
	// DCK mod, change above line for below with full cast
	if(ct && ct->flags != APC_UNCONSTRAINED) {
		/* #10.5.6 */
...
INTEGER_encode_uper(asn_TYPE_descriptor_t *td,
	...
	if(ct) {
		...
		if(specs && specs->field_unsigned) {
			...
			/* Check proper range */
			//if(ct->flags & APC_SEMI_CONSTRAINED) {
			// DCK mod, change above line to below for full cast
			if(ct->flags & APC_SEMI_CONSTRAINED) {
			...
			/* Check proper range */
			//if(ct->flags & APC_SEMI_CONSTRAINED) {
			// DCK mod, change above to below for full cast
			if(ct->flags & APC_SEMI_CONSTRAINED) {
