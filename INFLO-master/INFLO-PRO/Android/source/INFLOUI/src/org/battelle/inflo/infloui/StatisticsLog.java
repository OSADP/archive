/**
 * @file         infloui/ApplicationLog.java
...
public class StatisticsLog {
	...
	public StatisticsLog(String filename, boolean addTimeToFileName,
			String... headers) {

		if (Environment.getExternalStorageState().equals(
				Environment.MEDIA_MOUNTED)) {
			File baseDirectory = new File(
					Environment.getExternalStorageDirectory() + "/inflologs");
			...
			rLogFile = new File(baseDirectory
					+ "/"
					+ (addTimeToFileName ? rDateFormatter.format(Calendar
							.getInstance().getTime()) + "_" : "") + filename
					+ ".txt");
	...
	public synchronized void open() throws Exception {
		...
		if (this.rLogFile == null)
			throw new Exception(
					"Base Directory not initalized (is external SD card present?)");
