...
uper_open_type_put(asn_TYPE_descriptor_t *td, asn_per_constraints_t *constraints, void *sptr, asn_per_outp_t *po) {
	...
	for(bptr = buf, toGo = size; toGo;) {
		...
		// DCK mod, to add cast, as below
		if(per_put_many_bits(po, (uint8_t *)bptr, maySave * 8)) break; // DCK cast
...
uper_open_type_get_simple(asn_codec_ctx_t *ctx, asn_TYPE_descriptor_t *td,
	...
	do {
		...
		if(bufLen + chunk_bytes > bufSize) {
			...
			// buf = ptr;
			// DCK mod, as below to add cast
			buf = (uint8_t *)ptr; // DCK cast
		}
...
uper_ugot_refill(asn_per_data_t *pd) {
	...
	// DCK mod, cast as below
	uper_ugot_key *arg = (uper_ugot_key *)pd->refill_key; // DCK cast
