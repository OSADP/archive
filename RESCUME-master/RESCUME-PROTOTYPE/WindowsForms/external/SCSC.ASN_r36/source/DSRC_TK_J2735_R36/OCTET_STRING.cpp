 ...
 */
// DCK note: Many mods to add full casts to this file, you may want to
// reuse an older copy where this is all done rather then patch a new
// copy with each rebuild.  This file is "canned" and does not chnage.
//
#include "stdafx.h"
...
static asn_OCTET_STRING_specifics_t asn_DEF_OCTET_STRING_specs = {
	...
	//ASN_OSUBV_STR
	ASN_OSUBV_STR
	// DCK mod, above line to add cast name
};

// DCK mod, note casts needed below to make work 
...
OCTET_STRING_decode_ber(asn_codec_ctx_t *opt_codec_ctx,
	...
	// DCK mod, above line to be below line to handle cast
	enum asn_OS_Subvariant type_variant = specs->subvariant;

	ASN_DEBUG("Decoding %s as %s (frame %ld)",
		td->name,
		//(type_variant == ASN_OSUBV_STR) ?
		// DCK mod, above line to become below line to handle cast
		(type_variant ==  ASN_OSUBV_STR) ?
	...
	case 0:
		...
		if(tlv_constr) {
			 ...
			 */
			_CH_PHASE(ctx, 3);
			// DCK mod, was: if(type_variant == ASN_OSUBV_ANY && tag_mode != 1)
	...
	phase1:
	  ...
	  do {
			...
			&& ((const uint8_t *)buf_ptr)[1] == 0)
		{
			...
			// DCK mod, above line becomes below line to handle cast
			if(type_variant == ASN_OSUBV_ANY // DCK cast
		 ...
		 */
		switch(type_variant) {
		// DCK mod, was: case ASN_OSUBV_BIT:
		case ASN_OSUBV_BIT:  // DCK cast
			/* X.690: 8.6.4.1, NOTE 2 */
			/* Fall through */
		// dck mod, was: case ASN_OSUBV_STR:
		case ASN_OSUBV_STR:  // DCK cast
		default:
			if(sel) {
			...
			/* Fall through */
		// dck mod, was: case ASN_OSUBV_ANY:
		case ASN_OSUBV_ANY:  // DCK cast
		...
		// if(type_variant == ASN_OSUBV_ANY
		// DCK mod, above line becomes below line for cast
		if(type_variant == ASN_OSUBV_ANY
		&& (tag_mode != 1 || sel->cont_level))
			APPEND(buf_ptr, tlvl);
		sel->got += tlvl;
	...
	case 2:
		...
		if(len > 0) {
			// DCK mod, was: if(type_variant == ASN_OSUBV_BIT
			if(type_variant == ASN_OSUBV_BIT   // DCK cast
			&& sel->bits_chopped == 0) {
	...
	// DCK mod, above becomes below to handle full cast
	if(type_variant == ASN_OSUBV_BIT && st->size) {
...
OCTET_STRING_encode_der(asn_TYPE_descriptor_t *td, void *sptr,
	...
	BIT_STRING_t *st = (BIT_STRING_t *)sptr;
	// enum asn_OS_Subvariant type_variant = specs->subvariant;
	// DCK mod to allow full cast in line below
	enum asn_OS_Subvariant type_variant = specs->subvariant;
	int fix_last_byte = 0;
	 ...
	 */
	// if(type_variant != ASN_OSUBV_ANY || tag_mode == 1) {
	// DCK mod above line to becomes below due to cast
	if(type_variant != ASN_OSUBV_ANY || tag_mode == 1) {
		er.encoded = der_write_tags(td,
			//	(type_variant == ASN_OSUBV_BIT) + st->size,
			//tag_mode, type_variant == ASN_OSUBV_ANY, tag,
			// DCK mod, above two lines replaced with full cast in two lines below
				(type_variant == ASN_OSUBV_BIT) + st->size,
			tag_mode, type_variant == ASN_OSUBV_ANY, tag,
			cb, app_key);
		if(er.encoded == -1) {
		...
		// DCK mod, above replaced with below for full cast
		assert(type_variant != ASN_OSUBV_ANY || tag_mode != -1);
		er.encoded = 0;
	}
	 ...
	 */
	// if(type_variant == ASN_OSUBV_BIT) {
	// DCK mod, above line for below with full cast
	if(type_variant == ASN_OSUBV_BIT) {
...
OCTET_STRING_decode_uper(asn_codec_ctx_t *opt_codec_ctx,
	...
	default:
	// DCK mod to add full cast for each case below (5 times)
	case ASN_OSUBV_ANY:
	...
	case ASN_OSUBV_STR:
		...
		// DCK mod to add full cast below
		if(cval->flags & APC_CONSTRAINED)
	...
	ASN_DEBUG("PER Decoding %s size %ld .. %ld bits %d",
		...
		// DCK mod above line to become below line due to cast
		csiz->flags & APC_EXTENSIBLE ? "extensible" : "non-extensible",
		csiz->lower_bound, csiz->upper_bound, csiz->effective_bits);

	//if(csiz->flags & APC_EXTENSIBLE) {
	// DCK mod, line above replaced with below to add full cast
	if(csiz->flags & APC_EXTENSIBLE) {
...
OCTET_STRING_encode_uper(asn_TYPE_descriptor_t *td,
	...
	switch(specs->subvariant) {
	default:
	// DCK mod add full cast to case statements below (5 times in case)
	case ASN_OSUBV_ANY:
		_ASN_ENCODE_FAILED;
	case ASN_OSUBV_BIT:
		canonical_unit_bits = unit_bits = 1;
	...
	case ASN_OSUBV_STR:
		canonical_unit_bits = unit_bits = 8;
		//if(cval->flags & APC_CONSTRAINED)
		// DCK mod, above line changed to below to add full cast
		if(cval->flags & APC_CONSTRAINED)
			unit_bits = cval->range_bits;
		bpc = OS__BPC_CHAR;
		sizeinunits = st->size;
