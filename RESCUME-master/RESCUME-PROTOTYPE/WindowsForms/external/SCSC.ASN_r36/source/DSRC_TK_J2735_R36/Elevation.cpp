...
void Elevation_Init ( Elevation_t* theObj)
{
	if (theObj != NULL)
	{
		// Allocate memory needed on heap
		int bufSize = Elevation_BuffSize;
		theObj->buf = (uint8_t *) malloc(bufSize);
		assert(theObj->buf);
		theObj->size = bufSize;
		theObj->buf[0] = 0xF0; // the unknown value is set to 0xF000
		theObj->buf[1] = 0x0;
...
void Elevation_ToBlob ( Elevation_t* theObj, char* theBlob)
{
...
void Elevation_FromBlob ( Elevation_t* theObj, char* theBlob)
{
	if ((theObj != NULL) && (theBlob != NULL))
	{
	}
