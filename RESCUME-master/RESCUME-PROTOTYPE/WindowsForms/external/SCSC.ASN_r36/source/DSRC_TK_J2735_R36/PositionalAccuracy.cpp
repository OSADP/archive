...
void PositionalAccuracy_Init (PositionalAccuracy_t* theObj)
{
	if (theObj != NULL)
	{
		...
		int bufSize = PositionalAccuracy_BuffSize;  
		theObj->buf = (uint8_t *) malloc(bufSize);
		assert(theObj->buf);
		theObj->size = bufSize;
		// stuff 0xFF to start (this is the unavailable value for the inner elements)
		for(int i=0; i<bufSize; i++) theObj->buf[i] = 0xFF;
