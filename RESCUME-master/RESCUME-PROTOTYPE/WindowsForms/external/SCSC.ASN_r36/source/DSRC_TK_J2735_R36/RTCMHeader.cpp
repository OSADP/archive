...
void RTCMHeader_Init (RTCMHeader_t* theObj)
{
	if (theObj != NULL)
	{
		// allocate memory needed on heap
		int size = RTCMHeader_BuffSize;
		theObj->buf = (uint8_t *) malloc(size);
		assert(theObj->buf);
		theObj->size = size;
		// stuff zero to start (wasteful but helps in debug)
		for(int i=0; i<size; i++) theObj->buf[i] = 0;
...
void RTCMHeader_ToBlob (RTCMHeader_t* theObj, char* theBlob)
{
...
void RTCMHeader_FromBlob (RTCMHeader_t* theObj, char* theBlob)
{
	if ((theObj != NULL) && (theBlob != NULL))
	{
	}
