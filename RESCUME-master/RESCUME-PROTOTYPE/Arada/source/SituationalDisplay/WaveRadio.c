...
void waveRadio_txPacket(asn_TYPE_descriptor_t *msgType, void *msgStruct)
{
	WSMRequest request;
	// Fill request header information
	request.chaninfo.channel    = waveOptions.txChannel;
	request.chaninfo.rate       = waveOptions.txRate;
	request.chaninfo.txpower    = waveOptions.txPower;
	request.version             = 2;
	request.psid                = waveOptions.psid;
	...
	//request.expirytime = ? ;
	getMACAddr(request.srcmacaddr, waveOptions.txChannel);
	...
	if(rvalenc.encoded != -1)
	{
		...
		if(ret < 0)
		{
			DBG_ERR(DBGM_WR, printf("WaveRadio: Could not transmit packet\n"));
		}
		else
		{
			DBG(DBGM_WR, printf("WaveRadio: Transmitted packet\n"));
		}
	}
	else
	{
		DBG_ERR(DBGM_WR, printf("WaveRadio: Could not encode structure\n"));
	}
}

void waveRadio_txRawPacket(uint8_t *data, int dataLength)
{
	WSMRequest request;
	// Fill request header information
	request.chaninfo.channel    = waveOptions.txChannel;
...
WaveRxPacket *waveRadio_createRxPacket(asn_TYPE_descriptor_t *type, void *structure)
{
	...
	if (waveRadioNumberOfActivePackets > 15)
		DBG_WARN(DBGM_WR, printf("WaveRadio: Current number of active packets: %u\n", waveRadioNumberOfActivePackets));
