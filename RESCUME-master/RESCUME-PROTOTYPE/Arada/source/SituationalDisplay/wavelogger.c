...
//static FILE *logfile;
static uint16_t logport  =  23456; /*NOTE*/ 
...
void genhrlogfilename(char *ctimestr, char *hrlogfile) {
	char *token = NULL;
	char *str = NULL;static int check_min=0, first_filedone=0;
	int i;
	...
	sprintf(hrlogfile, "%04d%02d%02d_%02d%02d_%x_%04x_%s_%04d", year, month_num, date, hour, (min/10)*10 , file_psid, mod_depl_dev_id,ap_name,seq_num);
	first_filedone=1;
        check_min = (min/10)*10;
...
uint32_t new_putPsidbyLen(uint8_t *addr,uint32_t psid, int *retIdx)
{
   ...
   if (psid <= 0x7F) {
       ...
       *retIdx = 4;
       retPsid = ((retPsid&0x000000ff)<<24)|((retPsid&0x0000ff00)<<8)|((retPsid&0x00ff0000)>>8)|((retPsid&0xff000000)>>24);
   }
   	if(!BIGENDIAN)
...
int local_logging_client(int from_txrx, void *buf, double time,int usec, int capture_complete_packet, char *buff)
{
	...
	WSMHDR wsmhdr;uint32_t psidLen = 0;
...
//	static long diff = 0;
	...
	gettimeofday(&tv, NULL);
#ifndef WIN32
	sprintf(m, "<logtime> <seconds> %llu </seconds> <microseconds> %d </microseconds> </logtime> ", (uint64_t)tv.tv_sec, (uint32_t)tv.tv_usec);
#else
	...
	if (gps) {
		...
		if (gpscontents & GPS_LAT) {
			sprintf(m, "<latitude> %lf </latitude> <latdir> %c </latdir> ", gps->latitude, (gps->latitude < 0)? 'S' : 'N');
			sprintf(buf + i, "%s ", m);
			i += (strlen(m));
		}
		if (gpscontents & GPS_LON) {
			sprintf(m, "<longitude> %lf </longitude> <londir> %c </londir> ", gps->longitude, (gps->longitude < 0)? 'W' : 'E');
			sprintf(buf + i, "%s ", m);
			i += (strlen(m));
		}
		if (gpscontents & GPS_ALT) {
