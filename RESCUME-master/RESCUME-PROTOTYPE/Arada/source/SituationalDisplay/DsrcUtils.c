...
#include <math.h>
 ...
 #error 'GPS_INVALID_DATA isn't defined... '
#endif
...
uint32_t dsrc_getBsmVehicleId32(BasicSafetyMessage_t *bsm)
{
	...
	uint32_t vehicleId32;
	memcpy(&vehicleId32, bsm->blob1.buf+1, 4);
	return vehicleId32;
}
...
uint32_t dsrc_getTimVehicleId32(TravelerInformation_t *tim)
{
	...
	if (!tim->packetID)
		return 0;
	if (tim->packetID->size == 1)
		return *((uint8_t *)tim->packetID->buf);
	if (tim->packetID->size == 2)
		return *((uint16_t *)tim->packetID->buf);
	if (tim->packetID->size >= 4)
		return *((uint32_t *)tim->packetID->buf);
	return 0;
}
...
{
	struct DsrcPointToPathMatchResults results = {0};
	results.isPastPath = 1;
...
void dsrc_getShapePointSet(ShapePointSet_t *shape, int num, double *outLatitude, double *outLongitude)
{
	...
	Vector node;
	dsrc_createVectorFromOffset(shape->nodeList.list.array[num], &node);
	double xOffset = node.x;
	double yOffset = node.y;

	double latOffset = yOffset * 360.0 / (2 * PI * DSRC_POLAR_RADIUS);
	double longOffset = xOffset * 360.0 / (2 * PI * DSRC_EQUATORIAL_RADIUS * cos(baseLatitude * PI / 180));
