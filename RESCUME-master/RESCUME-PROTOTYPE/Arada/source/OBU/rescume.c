...
int ChannelAcess = CHACCESS_CONTINUOUS;
...
int RxpktProcess() {
	...
	if (!rxmsg.decode_status)
	{
		...
		if (packet)
		{
			rescumeApp_pushWaveMessage(packet);
...
/*	int cnt, mv = 0;
	...
	waveRadio_txPacket(&asn_DEF_BasicSafetyMessage, bsm);
...
void ReadGpsdata() {
    ...
    {*/
	    ...
	    if (gpssockfd > 0) {
		    ...
		    wsmgps.latitude = gps.latitude;
		    wsmgps.longitude = gps.longitude;
